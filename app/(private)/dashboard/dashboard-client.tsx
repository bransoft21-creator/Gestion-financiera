  ...
  return (
    <PremiumCard className="mb-8">
      <div className="flex items-center justify-between gap-3 px-5 pb-2 pt-5 sm:px-6 sm:pt-6">
        ...
        <Button asChild size="sm" variant="ghost" className="h-8 shrink-0 px-2 text-xs text-zinc-500 hover:bg-white/[0.06] hover:text-zinc-100">
      ...
      <PremiumCardContent className="pt-1">
        {status === "loading" && (
          <div className="space-y-2">
            {[0, 1].map((i) => (
              <div key={i} className="h-12 animate-pulse rounded-xl bg-white/[0.035]" />
            ))}
          </div>
        )}
        ...
        {status === "done" && items.length === 0 && (
          <div className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] px-3 py-3">
            ...
            <p className="text-sm text-zinc-500">No hay señales nuevas que pidan atención.</p>
          </div>
        )}
          ...
          <div className="space-y-2">
            {items.slice(0, 3).map((item) => (
              <Link
                ...
                className="group flex items-start gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] px-3 py-3 transition hover:border-white/15 hover:bg-white/[0.05]"
              >
                <span className={cn(
                  ...
                  item.tone === "neutral" && "bg-white/[0.06] text-zinc-500",
                )}>
...
export function DashboardClient() {
  ...
  return (
    <div className="fade-in">
      ...
      {/* Preview pequeño del Financial Activity Center */}
      <ActivityPreview />
