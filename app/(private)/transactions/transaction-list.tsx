            ...
            {transaction.sharedTransaction ? (
              transaction.isHouseholdPayment && transaction.userShareAmount != null ? (
                <span
                  ...
                  title={`Saldo del hogar: ${formatMoney(Number(transaction.amount) - Number(transaction.userShareAmount), transaction.currency)} · Tu parte: ${formatMoney(Number(transaction.userShareAmount), transaction.currency)} · Total: ${formatMoney(Number(transaction.amount), transaction.currency)}`}
                >
                  <Home className="h-3 w-3" aria-hidden="true" />
                  {formatMoney(Number(transaction.amount) - Number(transaction.userShareAmount), transaction.currency)} en balance
                </span>
              ) : (
                <span
                  ...
                  title={transaction.sharedTransaction.household.name}
                >
                  <Home className="h-3 w-3" aria-hidden="true" />
                  Compartido
                </span>
